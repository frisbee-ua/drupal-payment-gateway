...
namespace Drupal\commerce_fondy\Element;
...
class Country extends FormElement
{
    ...
    public static function processCountry(array &$element, FormStateInterface $form_state, array &$complete_form)
    {
        ...
        if (count($country_list) == 1 && $element['#required']) {
            ...
            if (! $element['#required']) {
                $element[$elementId]['#empty_value'] = '';
            }
            if (! empty($element['#ajax'])) {
                $element[$elementId]['#ajax'] = $element['#ajax'];
                unset($element['#ajax']);
            }
            if (! empty($element['#description'])) {
                $element[$elementId]['#description'] = $element['#description'];
                unset($element['#description']);
            }
            if (! empty($element['#multiple'])) {
                $element[$elementId]['#multiple'] = $element['#multiple'];
            }
        }
        // Remove the 'country_code' level from form state values.
        $element[$elementId]['#parents'] = $element['#parents'];
